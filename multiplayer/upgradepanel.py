...
class UpgradePanel(Panel):
    def __init__(self, pos, civ, offered_upgrades, resource, on_select, on_reroll):
        ...
        self.tab = {"text":"Up", "color":economy.RESOURCE_COLORS[self.resource], "icon":"assets/i-upgrade.png"}

        y = 0
        # need fn for closure
        def add_button(uname):
            u = upgrades.UPGRADE_CLASSES[uname]
            b = UpgradeIcon(V2(0,0), uname, lambda u:on_select(u), True)
            self.add(b, V2(0,y))
            return b
        ...
        self.joystick_controls.extend([[c] for c in self.get_controls_of_type(UpgradeIcon)])
